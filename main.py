...
def parse_args():
    ...
    parser.add_argument('--checkpoint_path_continue_training', dest='checkpoint_path_continue_training', help='The path to read the checkpoint when continue training ',
        default='./trained_save/')
    parser.add_argument('--root', type=Path, default='/local2/jiajunb/MAC/')
...
def run_training():
    ...
    train_feature_dir = args.root / "charades_sta_visual_feature/all_fc6_unit16_overlap0.5"
    test_feature_dir = args.root / "charades_sta_visual_feature/all_fc6_unit16_overlap0.5"


    # visual activity concepts for each video clip
    # the softmax layer ouput of R(2+1)D pretraiend on Kinetics, 400-demension
    train_softmax_dir = args.root / 'charades_sta_visual_activity_concepts/train_softmax'
    test_softmax_dir = args.root / 'charades_sta_visual_activity_concepts/test_softmax'
    ...
    train_clip_sentence_pairs_iou_path = args.root / "ref_info/charades_sta_train_semantic_sentence_VP_sub_obj.pkl"
    test_clip_sentence_pairs_path = args.root / 'ref_info/charades_sta_test_semantic_sentence_VP_sub_obj.pkl'
    ...
    # 8: others; 9: others
    test_swin_txt_path = args.root / "ref_info/charades_sta_test_swin_props_num_36364.txt"
    ...
    if is_only_test:
        ...
        with tf.Graph().as_default():
            ...
            movie_length_dict={}
            with open(args.root / "ref_info/charades_movie_length_info.txt") as f:
                for l in f:
